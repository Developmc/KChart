...
public class BarChartRenderer extends BarLineScatterCandleBubbleRenderer {
    ...
    @Override public void initBuffers() {
        ...
        for (int i = 0; i < mBarBuffers.length; i++) {
            ...
            mBarBuffers[i] =
                new BarBuffer(set.getEntryCount() * 4 * (set.isStacked() ? set.getStackSize() : 1),
    ...
    protected void drawDataSet(Canvas c, IBarDataSet dataSet, int index) {
        ...
        for (int j = 0; j < buffer.size(); j += 4) {
            ...
            if (!isSingleColor) {
                ...
                //edit by clement 2018.04.20 自定义规则：给柱状图添加颜色，红色表示上涨，绿色表示下跌(颜色从外部传入)
                int i = j / 4;
                if (i > 0) {
                    if (dataSet.getEntryForIndex(i).getY() > dataSet.getEntryForIndex(i - 1)
                        .getY()) {
    ...
    @Override public void drawValues(Canvas c) {
        ...
        if (isDrawingValuesAllowed(mChart)) {
            ...
            for (int i = 0; i < mChart.getBarData().getDataSetCount(); i++) {
                ...
                posOffset =
                    (drawValueAboveBar ? -valueOffsetPlus : valueTextHeight + valueOffsetPlus);
                negOffset =
                ...
                if (!dataSet.isStacked()) {
                    ...
                    for (int j = 0; j < buffer.buffer.length * mAnimator.getPhaseX(); j += 4) {
                        ...
                        if (!mViewPortHandler.isInBoundsY(buffer.buffer[j + 1]) || !mViewPortHandler
                            .isInBoundsLeft(x)) {
                            continue;
                        }
                        ...
                        if (dataSet.isDrawValuesEnabled()) {
                            drawValue(c, dataSet.getValueFormatter(), val, entry, i, x,
                                val >= 0 ? (buffer.buffer[j + 1] + posOffset)
                        ...
                        if (entry.getIcon() != null && dataSet.isDrawIconsEnabled()) {
                            ...
                            float px = x;
                            float py = val >= 0 ? (buffer.buffer[j + 1] + posOffset)
                                : (buffer.buffer[j + 3] + negOffset);

                            px += iconsOffset.x;
                            py += iconsOffset.y;

                            Utils.drawImage(c, icon, (int) px, (int) py, icon.getIntrinsicWidth(),
                        ...
                        if (vals == null) {
                            ...
                            if (dataSet.isDrawValuesEnabled()) {
                                drawValue(c, dataSet.getValueFormatter(), entry.getY(), entry, i, x,
                                    buffer.buffer[bufferIndex + 1] + (entry.getY() >= 0 ? posOffset
                                        : negOffset), color);
                            }
                            ...
                            for (int k = 0; k < transformed.length; k += 2) {
                                ...
                                if (entry.getIcon() != null && dataSet.isDrawIconsEnabled()) {
                                    ...
                                    Utils.drawImage(c, icon, (int) (x + iconsOffset.x),
                                        (int) (y + iconsOffset.y), icon.getIntrinsicWidth(),
                                        icon.getIntrinsicHeight());
                                }
                            }
                        }
    ...
    @Override public void drawHighlighted(Canvas c, Highlight[] indices) {
        ...
        for (Highlight high : indices) {
            ...
            //edit by clement 2018.04.20 将绘制的矩形修改为高亮的线
            //c.drawRect(mBarRect, mHighlightPaint);
            float xValue = mBarRect.left + (mBarRect.right - mBarRect.left) / 2;
            //startX,startY,stopX,stopY,paint
            c.drawLine(xValue, this.mViewPortHandler.contentTop(), xValue,
                this.mViewPortHandler.contentBottom(), this.mHighlightPaint);
        }
    }
