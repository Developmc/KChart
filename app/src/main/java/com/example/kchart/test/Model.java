 ...
 */
public class Model {
    @Deprecated public static List<StockListBean.StockBean> getData() {
    ...
    public static List<BtcData> getBtcData(KLineType type) {
        if (type == KLineType.MINUTE) {
            return new Gson().fromJson(btcMinuteData, Btc.class).getData();
        } else if (type == KLineType.HOUR) {
    ...
    private static List<CandleEntry> getCandleEntries(List<BtcData> rawData, int startIndex) {
        ...
        for (int i = 0; i < rawData.size(); i++) {
            ...
            CandleEntry entry = new CandleEntry(startIndex + i, btcData.getHigh(), btcData.getLow(),
                btcData.getOpen(), btcData.getClose());
            entries.add(entry);
        }
    ...
    public static List<Entry> getSellDepths() {
        List<Depth> depths = new ArrayList<>();
        depths.add(new Depth(0.07162f, 23100, 13100, "sell"));
        depths.add(new Depth(0.07163f, 60000, 73100, "sell"));
        depths.add(new Depth(0.071681f, 40250, 73350, "sell"));
        depths.add(new Depth(0.07171f, 22804, 196154, "sell"));
        depths.add(new Depth(0.0718f, 50000, 246154, "sell"));
        //按照price排序
        Collections.sort(depths, new Comparator<Depth>() {
            @Override public int compare(Depth o1, Depth o2) {
                return o1.getPrice() < o2.getPrice() ? -1 : 1;
            }
        });
        List<Entry> entries = new ArrayList<>();
        for (Depth depth : depths) {
            entries.add(new Entry(depth.getPrice(), depth.getAmount()));
        }
        return entries;
    }

    public static List<Entry> getBuyDepths() {
        List<Depth> depths = new ArrayList<>();
        depths.add(new Depth(0.07151f, 74588, 194588, "buy"));
        depths.add(new Depth(0.0715f, 63827, 1488415, "buy"));
        depths.add(new Depth(0.07149f, 20000, 1490415, "buy"));
        depths.add(new Depth(0.07148f, 43400, 1533815, "buy"));
        depths.add(new Depth(0.0714f, 52600, 1656415, "buy"));
        //按照price排序
        Collections.sort(depths, new Comparator<Depth>() {
            @Override public int compare(Depth o1, Depth o2) {
                return o1.getPrice() < o2.getPrice() ? -1 : 1;
